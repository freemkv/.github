...
def _selftest():
    ...
    cases = [
        ...
        # a string literal ENDING in `r`/`br` (e.g. "fr", "pt-br") must NOT be
        # misread as a raw-string opener — else the parser would think it's inside
        # a string for the rest of the file and miss the 4-line inline block below
        (
            'let x = "fr";\nlet y = "pt-br";\n'
            + "// a\n// b\n// c\n// d\nfn p() {}\n",
            1,
        ...
        # an explicit BYTE raw string (br#"…"#) is string data, not comments
        (
            'const B: &[u8] = br#"<x>\n'
            + "".join("// note\n" for _ in range(8))
            + '</x>"#;\nfn b() {}\n',
